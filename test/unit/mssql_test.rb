...
describe Labrador::Mssql do
  ...
  describe '#collections' do
    it "should list collections/tables" do
      assert_equal ["users"], @mssql.collections
    end
  end

  describe '#primary_key_for' do
    it "should find primary key for collection/table" do
      assert_equal 'id', @mssql.primary_key_for(:users)
    end
  end
  ...
  describe '#find' do
    ...
    describe 'with limit' do
      it 'should find records' do
        results = @mssql.find(:users, limit: 20)
    ...
    describe 'with offset/skip' do
      it 'should find records' do
        results = @mssql.find(:users, skip: 10)
    ...
    describe 'with order_by and direction' do
      it 'should find records' do
        results = @mssql.find(:users, order_by: 'username', direction: 'asc', limit: 1)
        assert_equal 'user1', results.first["username"]
        results = @mssql.find(:users, order_by: 'username', direction: 'desc', limit: 1)
    ...
    describe '#fields_for' do
      it 'should find fields given results' do
        assert_equal ["id", "username", "age"], @mssql.fields_for(@mssql.find(:users))
      end
    end
  end
  ...
  describe '#create' do
    ...
    it 'insert a new record into the collection' do
      assert_equal @previousCount + 1, @mssql.find(:users, limit: 1000).count
    end
  ...
  describe '#update' do
    ...
    it 'should maintain collection count after update' do
      assert_equal @previousCount , @mssql.find(:users, limit: 1000).count
    end
